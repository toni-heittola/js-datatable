...
toolbar_html+='<label class="'+this.options.button_css.off+active+'" data-mode="off">';toolbar_html+='<input autocomplete="off" checked data-mode="off" name="options" type="radio" value="off">'+this.options.icon.off+' '+this.options.label.off+'</input>';toolbar_html+='</label>';toolbar_html+='</div>';toolbar_html+='</div>';$(toolbar_html).insertBefore(this.element);$(this.element).attr('data-toolbar','#datatable_toolbar'+this.uniqueId);this.options.element.chart_mode_selector='#chart_mode_selector'+this.uniqueId;if(this.options.chart_modes.indexOf('bar')>-1){var bar_div='<div id="bar_div'+this.uniqueId+'" style="display:none;"><canvas id="bar_chart'+this.uniqueId+'"></canvas></div>';this.options.element.bar.div='#bar_div'+this.uniqueId;this.options.element.bar.canvas='#bar_chart'+this.uniqueId;$($div).append(bar_div);}
...
var value_label=header.text().trim();var labels=[];var data=[];var border_colors=[];var border_hover_colors=[];var bg_colors=[];var bg_hover_colors=[];this.hline_value=[];this.hline_color=[];this.hline_index=[];for(var i=0;i<table_data.length;i++){var label=table_data[i][this.options.table.id_field].trim().replace(/<(?:.|\n)*?>/gm,'').replace('_',' ');if(labels.indexOf(label)>-1){var num_of_occurances=0;for(var i=0;i<labels.length;i++){if(labels[i]===label)
num_of_occurances++;}
...
values[value]++;});}});var sortable=[];for(var value_group in values){sortable.push([value_group,values[value_group]]);sortable.sort(function(a,b){return b[1]-a[1]})}
$.each(sortable,function(i,val){map[val[0]]=i;});$.each(this.field_meta,function(index,meta){if(meta.tag){$(self.element).find('tbody tr').each(function(){var value=$(this).find('td').eq(index).text().trim();var tag_css=self.options.tags.css[map[value]];$(this).find('td').eq(index).html('<span class="'+tag_css+'">'+value+'</span>');})}})}else if(this.options.tag_mode=='column'){var map={};$.each(this.field_meta,function(index,meta){if(meta.tag){var values={};$(self.element).find('tbody tr').each(function(){var value=$(this).find('td').eq(index).text().trim();if(!(value in values)){values[value]=0;}
values[value]++;});var sortable=[];for(var value_group in values){sortable.push([value_group,values[value_group]]);sortable.sort(function(a,b){return b[1]-a[1]})}
