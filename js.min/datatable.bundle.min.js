...
toolbar_html+='>';toolbar_html+=this.options.icon.table+' '+this.options.label.table;toolbar_html+='</button>&nbsp;';toolbar_html+='<div class="btn-group" data-toggle="buttons" id="'+this.options.element.chart_mode_selector+'">';var $div=$('<div>',{id:'datatable_visualization'+this.uniqueId,'class':'datatable'});for(var mode_id=0;mode_id<this.options.chart_modes.length;mode_id++) {var current_mode_name=this.options.chart_modes[mode_id]
var current_mode_config={};var custom_found=false;var active='';if(this.options.chart_default_mode==current_mode_name){active=' active ';}
...
break;}}
if(custom_found){var graph_type=current_mode_config['type'];var active='';if(this.options.chart_default_mode==current_mode_name){active=' active ';}
var icon='';if(current_mode_config.hasOwnProperty('icon')){icon=current_mode_config['icon'];}else{icon=this.options.icon[graph_type];}
var button_css='';if(current_mode_config.hasOwnProperty('button_css')){button_css=this.options.button_css.custom+' '+current_mode_config['button_css'];}else{button_css=this.options.button_css[graph_type];}
...
this.line_chart=new Chart(chart_ctx,{type:'line',data:chart_data,options:chart_options});this.line_chart_data=chart_data;}else{if($(this.options.element.line.div).is(":visible")){$(this.options.element.line.div).hide(0);}}},showLineChart:function(){var self=this;if(this.validLine()){if($(this.options.element.line.div).is(":hidden")){$(this.options.element.line.div).show(this.options.animation.show_speed,function(){self.updateLine();});}}},hideLineChart:function(){if($(this.options.element.line.div).is(":visible")){$(this.options.element.line.div).hide(this.options.animation.hide_speed,function(){});}},getLineChartParameters:function(data){var parameters=jQuery.extend(true,{},this.defaults.line);parameters.fields=this.getParameterItem(data,'line-fields');if(parameters.fields){parameters.fields=parameters.fields.split(',').map(function(item){return item.trim();});}
parameters.fields=this.getParameterItem(data,'line-extra-fields');if(parameters.fields){parameters.fields=parameters.fields.split(',').map(function(item){return item.trim();});}
...
var stacked_chart=false;var stacked_items=1;if(options[type].fields.length==1){for(var field_id=0;field_id<options[type].fields.length;field_id++){for(var row_id=0;row_id<table_data.length;row_id++){var values=self.parseDataList(table_data[row_id][options[type].fields[field_id]]);if(values.length>1){stacked_chart=true;if(values.length>stacked_items){stacked_items=values.length;}}}}}
var datasets=[];var all_value_max=-Infinity;var all_value_min=Infinity;var beginAtZero=options[type].yaxis.beginatzero;if(stacked_chart){if(header_options.dataset.labels.length==0){header_options.dataset.labels=new Array(stacked_items);for(var row_id=0;row_id<table_data.length;row_id++){var values=self.parseDataList(table_data[row_id][sort_name]);for(var value_id=0;value_id<values.length;value_id++){if(values[value_id].label){header_options.dataset.labels[value_id]=values[value_id].label;}}}}
var current_header_options=this.getHeaderOptions(options[type].fields[0]);if(current_header_options.chartable){if(!beginAtZero){beginAtZero=current_header_options.beginatzero;}
if(current_header_options.axis.scale){if(current_header_options.axis.scale.startsWith('log')){axis_scale='logarithmic';axis_scale_type='log';if(['log_unit','log_exp','log10','log10_unit','log10_exp'].includes(current_header_options.axis.scale)){axis_scale_type=current_header_options.axis.scale;}}}
for(var dataset_id=0;dataset_id<stacked_items;dataset_id++){var dataset={label:header_options.dataset.labels[dataset_id],value_postfix:current_header_options.value.postfix,data:[],data_uniqueid:[],data_min:[],data_max:[],data_string:[],borderWidth:2,borderColor:this.colorCode(options[type].colors.datasets[dataset_id]),lineTension:0,fill:true,backgroundColor:this.colorCode(options[type].colors.datasets[dataset_id],0.6),pointRadius:[],pointHoverRadius:[]};for(var row_id=0;row_id<table_data.length;row_id++){var values=self.parseDataList(table_data[row_id][sort_name]);var value_sum=0;for(var ii=0;ii<values.length;ii++){value_sum+=values[ii]['y_value'];}
var item={'value':null,'value_max':null,'value_min':null};if(values.length>dataset_id&&values[dataset_id]['y_value']){var percentage=values[dataset_id]['y_value'];if(value_sum>100.0&&value_sum<101.0){percentage=percentage/value_sum*100.0;}
item.value=percentage;}
if(current_header_options.value.formatter){var value_string=this.stripHTML(window[current_header_options.value.formatter](item.value));if(current_header_options.value.pad){value_string=(value_string+current_header_options.value.pad).substring(0,current_header_options.value.pad.length);}
dataset.data_string.push(value_string);}else{dataset.data_string.push(this.injectValuePostfix(item.value,current_header_options.value.postfix));}
dataset.data_uniqueid.push(table_data[row_id][table_options.idField]);if(time_mode){var time_label=$("<div/>").html(table_data[row_id][xaxis_field]).text().trim().replace(/<(?:.|\n)*?>/gm,'').replace('_',' ');dataset.data.push({x:time_label,y:item.value});}else{dataset.data.push(item.value);}
if(options[type].point.enabled){dataset.pointRadius.push(options[type].point.radius.normal);dataset.pointHoverRadius.push(options[type].point.radius.hover);}else{dataset.pointRadius.push(0);dataset.pointHoverRadius.push(0);}}
datasets.push(dataset);}}}else{var dataset_id=0;for(var field_id=0;field_id<options[type].fields.length;field_id++){var current_header_options=this.getHeaderOptions(options[type].fields[field_id]);if(current_header_options.chartable){if(!beginAtZero){beginAtZero=current_header_options.beginatzero;}
var valid_dataset=false;if(options[type].fields){if(options[type].fields.includes(current_header_options.field)){valid_dataset=true;}}else{valid_dataset=true;}
if(valid_dataset){if(current_header_options.axis.scale){if(current_header_options.axis.scale.startsWith('log')){axis_scale='logarithmic';axis_scale_type='log';if(['log_unit','log_exp','log10','log10_unit','log10_exp'].includes(current_header_options.axis.scale)){axis_scale_type=current_header_options.axis.scale;}}}
var dataset={label:current_header_options.title,value_postfix:current_header_options.value.postfix,data:[],data_uniqueid:[],data_min:[],data_max:[],data_string:[],horizontal_line_value:[],borderWidth:2,borderColor:this.colorCode(options[type].colors.datasets[dataset_id]),lineTension:0,spanGaps:true,fill:options[type].fill,backgroundColor:this.colorCode(options[type].colors.datasets[dataset_id],0.10),pointRadius:[],pointHoverRadius:[],pointBorderColor:[],pointHoverBorderColor:[],pointBackgroundColor:[],pointHoverBackgroundColor:[]};for(var row_id=0;row_id<table_data.length;row_id++){if(current_header_options.value.formatter){dataset.data_string.push(self.stripHTML(window[current_header_options.value.formatter](table_data[row_id][current_header_options.field])));}else{dataset.data_string.push(self.injectValuePostfix(table_data[row_id][current_header_options.field],current_header_options.value.postfix));}
dataset.data_uniqueid.push(table_data[row_id][table_options.idField]);var item=self.parseNumericValue(table_data[row_id][current_header_options.field]);if(time_mode){dataset.data.push({x:$("<div/>").html(table_data[row_id][xaxis_field]).text().trim().replace(/<(?:.|\n)*?>/gm,'').replace('_',' '),y:item.value});}else{dataset.data.push(item.value);}
var value_max=item.value_max;var value_min=item.value_min;if(value_min!==null&&value_max!==null){if(value_max<value_min){var value_tmp=value_min;value_min=value_max;value_max=value_tmp;}
if(all_value_max<value_max){all_value_max=value_max;}
if(all_value_min>value_min){all_value_min=value_min;}}
dataset.data_min.push(value_min);dataset.data_max.push(value_max);if(table_data[row_id].hasOwnProperty('_data')&&table_data[row_id]['_data'].hasOwnProperty('hline')&&typeof table_data[row_id]['_data']['hline']!=='undefined'&&table_data[row_id]['_data']['hline']){dataset.horizontal_line_value.push(item.value);}
if(table_data[row_id].hasOwnProperty('_class')&&typeof table_data[row_id]['_class']!=='undefined'){if(self.options[type].colors.valid.indexOf(table_data[row_id]['_class'])>-1){dataset.pointBorderColor.push(self.options[type].colors[table_data[row_id]['_class']].border.normal);dataset.pointHoverBorderColor.push(self.options[type].colors[table_data[row_id]['_class']].border.hover);dataset.pointBackgroundColor.push(self.options[type].colors[table_data[row_id]['_class']].bg.normal);dataset.pointHoverBackgroundColor.push(self.options[type].colors[table_data[row_id]['_class']].border.hover);}else{dataset.pointBorderColor.push(self.hexToRGB(options[type].colors.datasets[dataset_id],0.5).css);dataset.pointHoverBorderColor.push(self.hexToRGB(options[type].colors.datasets[dataset_id],0.8).css);dataset.pointBackgroundColor.push(self.hexToRGB(options[type].colors.datasets[dataset_id],0.6).css);dataset.pointHoverBackgroundColor.push(self.hexToRGB(options[type].colors.datasets[dataset_id],1.0).css);}}else{dataset.pointBorderColor.push(self.hexToRGB(options[type].colors.datasets[dataset_id],0.5).css);dataset.pointHoverBorderColor.push(self.hexToRGB(options[type].colors.datasets[dataset_id],0.8).css);dataset.pointBackgroundColor.push(self.hexToRGB(options[type].colors.datasets[dataset_id],0.6).css);dataset.pointHoverBackgroundColor.push(self.hexToRGB(options[type].colors.datasets[dataset_id],1.0).css);}
if(options[type].point&&options[type].point.enabled){dataset.pointRadius.push(options[type].point.radius.normal);dataset.pointHoverRadius.push(options[type].point.radius.hover);}else{dataset.pointRadius.push(0);dataset.pointHoverRadius.push(0);}}
datasets.push(dataset);dataset_id++;}}}}
if(datasets.length>0){if(!axis_scale_type){if(options[type].yaxis.scale){if(options[type].yaxis.scale.startsWith('log')){axis_scale='logarithmic';axis_scale_type='log';if(['log_unit','log_exp','log10','log10_unit','log10_exp'].includes(options[type].yaxis.scale)){axis_scale_type=options[type].yaxis.scale;}}}}
$(this.options.custom_chart[name].div).show(0);var chart_data={labels:labels,datasets:datasets};var show_legend=false;if(datasets.length>1&&options[type].legend.enabled){show_legend=true;}
var tooltip_position='average';var padding={left:0,right:0,top:0,bottom:0};if(options[type].tooltips.position==='top'){tooltip_position='line_centered_top';padding={left:0,right:0,top:40+datasets.length*12,bottom:0};}else if(options[type].tooltips.position==='bottom'){tooltip_position='line_centered_bottom';padding={left:0,right:0,top:0,bottom:40+datasets.length*12};}
var chart_options={showLines:true,elements:{line:{tension:1,}},layout:{padding:padding},scales:{xAxes:[{display:options[type].xaxis.display,stacked:false,ticks:{maxRotation:90},gridLines:{display:options[type].xaxis.gridlines}}],yAxes:[{stacked:stacked_chart,type:axis_scale,ticks:{beginAtZero:beginAtZero,suggestedMin:all_value_min,suggestedMax:all_value_max},gridLines:{display:options[type].yaxis.gridlines}}]},maintainAspectRatio:true,responsive:true,animation:{duration:0},hover:{animationDuration:0},responsiveAnimationDuration:0,title:{display:false},legend:{display:show_legend,position:options[type].legend.position},horizontal_highlights:options[type].horizontal_highlights,horizontal_line:options[type].horizontal_line,vertical_indicator_line:options[type].vertical_indicator_line,vertical_segments:options[type].vertical_segments,horizontal_indicator_line:options[type].horizontal_indicator_line,interval:options[type].interval,error_bar:options[type].error_bar,tooltips:{enabled:true,mode:'label',position:tooltip_position,intersect:false,caretSize:0,caretPadding:0,titleFontSize:options[type].tooltips.titleFontSize,bodyFontSize:options[type].tooltips.bodyFontSize,bodyFontFamily:options[type].tooltips.bodyFontFamily,callbacks:{afterBody:function(tooltipItems,data){$(self.element).find('tr td').removeClass('row-hover');var table_options=$(self.element).bootstrapTable('getOptions');for(var i=0;i<tooltipItems.length;i++){var value_index=tooltipItems[i].index;$(self.element).find('tbody td:contains('+table_options.data[value_index][table_options.idField]+')').parent('tr').find('td').addClass('row-hover')}},label:function(tooltipItems,data){var data_string=data.datasets[tooltipItems.datasetIndex].data_string[tooltipItems.index];return data_string+' ('+data.datasets[tooltipItems.datasetIndex].label+')';}}},chartArea:{backgroundColor:options[type].colors.background},onHover:function(event,elements){if(event.type=='mouseout'||event.type=='touchend'){$(self.element).find('tr td').removeClass('row-hover');}else if(event.type=='mousemove'){if(elements.length>0){$(self.element).find('tr td').removeClass('row-hover');var value_index=elements[0]._index;var table_options=$(self.element).bootstrapTable('getOptions');$(self.element).find('tbody td:contains('+table_options.data[value_index][table_options.idField]+')').parent('tr').find('td').addClass('row-hover')}}}};if(time_mode){chart_options.scales.xAxes[0].type='time';chart_options.scales.xAxes[0].distribution='linear';chart_options.scales.xAxes[0].time={};chart_options.scales.xAxes[0].time.displayFormats=options.time.display_formats;if(options[type].xaxis.timeunit){chart_options.scales.xAxes[0].time.unit=options[type].xaxis.timeunit;}
if(options[type].xaxis.timestepsize){chart_options.scales.xAxes[0].time.stepSize=options[type].xaxis.timestepsize;}
chart_options.scales.xAxes[0].ticks={};}
if(options[type].yaxis.label){chart_options.scales.yAxes[0].scaleLabel={display:true,labelString:options[type].yaxis.label,fontColor:'black'}}
if(axis_scale==='logarithmic'){chart_options.scales.yAxes[0].ticks.callback=function(value,index,values){if(axis_scale_type.startsWith('log10')){if(!(value===1e1||value===1e2||value===1e3||value===1e4||value===1e5||value===1e6||value===1e7||value===1e8||value===1e9||value===1e10)){return'';}}else if(axis_scale_type.startsWith('log')){if(values.length>10){if(index%Math.ceil(values.length/10.0)!==0){return'';}}}
if(axis_scale_type.endsWith('_unit')){return self.addNumberPrefix(value);}else if(axis_scale_type.endsWith('_exp')){return value.toExponential(2);}else{return value;}}}
if(stacked_chart){chart_options.interval.enabled=false;chart_options.horizontal_indicator_line.enabled=false;}
this.options.custom_chart[name].chart=new Chart(chart_ctx,{type:type,data:chart_data,options:chart_options});this.options.custom_chart[name].data=chart_data;}else{if($(this.options.custom_chart[name].div).is(":visible")){$(this.options.custom_chart[name].div).hide(0);}}},showCustomChart:function(name){var self=this;if($(this.options.custom_chart[name].div).is(":hidden")){$(this.options.custom_chart[name].div).show(this.options.animation.show_speed,function(){self.updateCustomChart(name);});}},hideCustomChart:function(name){if($(this.options.custom_chart[name].div).is(":visible")){$(this.options.custom_chart[name].div).hide(this.options.animation.hide_speed,function(){});}},updateTags:function(){var self=this;if(this.options.tag_mode=='global'){var map={};var values={};jQuery.each(this.field_meta,function(index,meta){if(meta.tag){$(self.element).find('tbody tr').each(function(){var value=$(this).find('td').eq(index).text().trim();value.split(',').forEach(function(item){if(!(item.trim()in values)){values[item.trim()]=0;}
...
function valueFormatter_int_plus(value,row,index){return valueFormatter_generic_fixed(value,row,index,0,false,undefined,undefined,true);}
function valueFormatter_float1_plus(value,row,index){return valueFormatter_generic_fixed(value,row,index,1,false,undefined,undefined,true);}
function valueFormatter_float2_plus(value,row,index){return valueFormatter_generic_fixed(value,row,index,2,false,undefined,undefined,true);}
function valueFormatter_float3_plus(value,row,index){return valueFormatter_generic_fixed(value,row,index,3,false,undefined,undefined,true);}
function valueFormatter_float4_plus(value,row,index){return valueFormatter_generic_fixed(value,row,index,4,false,undefined,undefined,true);}
...
function valueFormatter_int_percentage_muted(value,row,index){return valueFormatter_generic_fixed(value,row,index,0,true,undefined,' %');}
...
function valueFormatter_float1_percentage_muted(value,row,index){return valueFormatter_generic_fixed(value,row,index,1,true,undefined,' %');}
...
function valueFormatter_float2_percentage_muted(value,row,index){return valueFormatter_generic_fixed(value,row,index,2,true,undefined,' %');}
...
function valueFormatter_float3_percentage_muted(value,row,index){return valueFormatter_generic_fixed(value,row,index,3,true,undefined,' %');}
...
function valueFormatter_float4_percentage_muted(value,row,index){return valueFormatter_generic_fixed(value,row,index,4,true,undefined,' %');}
function valueFormatter_int_db(value,row,index){return valueFormatter_generic_fixed(value,row,index,0,false,undefined,' dB');}
function valueFormatter_int_db_muted(value,row,index){return valueFormatter_generic_fixed(value,row,index,0,true,undefined,' dB');}
function valueFormatter_float1_db(value,row,index){return valueFormatter_generic_fixed(value,row,index,1,false,undefined,' dB');}
function valueFormatter_float1_db_muted(value,row,index){return valueFormatter_generic_fixed(value,row,index,1,true,undefined,' dB');}
function valueFormatter_float2_db(value,row,index){return valueFormatter_generic_fixed(value,row,index,2,false,undefined,' dB');}
function valueFormatter_float2_db_muted(value,row,index){return valueFormatter_generic_fixed(value,row,index,2,true,undefined,' dB');}
function valueFormatter_float3_db(value,row,index){return valueFormatter_generic_fixed(value,row,index,3,false,undefined,' dB');}
function valueFormatter_float3_db_muted(value,row,index){return valueFormatter_generic_fixed(value,row,index,3,true,undefined,' dB');}
function valueFormatter_float4_db(value,row,index){return valueFormatter_generic_fixed(value,row,index,4,false,undefined,' dB');}
function valueFormatter_float4_db_muted(value,row,index){return valueFormatter_generic_fixed(value,row,index,4,true,undefined,' dB');}
...
function valueFormatter_html(value,row,index){return value.replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&amp;/g,'&');}
